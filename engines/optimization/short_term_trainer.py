import os
import json
import pandas as pd
import yfinance as yf
import numpy as np
from datetime import datetime, timedelta
import warnings

warnings.filterwarnings('ignore')

import sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))

from engines.bull.relative_strength import RelativeStrengthEngine
from engines.bull.momentum import MomentumBullEngine
from core.types import DataPacket, Signal, SignalCategory, SignalSubSource

print("=== 3-MONTH SHORT-TERM GRID SEARCH ===")
print("Downloading highly granular data for Nifty heavyweights...")

stocks = ["RELIANCE.NS", "HDFCBANK.NS", "INFY.NS", "LT.NS", "ITC.NS", "SBIN.NS", "TCS.NS", "BHARTIARTL.NS"]
# We fetch 1 year of history to provide the 63-day lookback needed for the last 3 months of target dates
nifty = yf.download("^NSEI", start="2025-06-01", end="2026-06-06", progress=False)['Close']
if isinstance(nifty, pd.DataFrame): nifty = nifty.iloc[:, 0]

stock_data = {}
for t in stocks:
    df = yf.download(t, start="2025-06-01", end="2026-06-06", progress=False)['Close']
    if isinstance(df, pd.DataFrame): df = df.iloc[:, 0]
    stock_data[t] = df

# Test dates: every Friday in the past 3 months
end_date = datetime(2026, 6, 5)
start_date = end_date - timedelta(days=90)
sample_dates = pd.date_range(start=start_date, end=end_date - timedelta(days=10), freq="W-FRI")

print(f"Executing Grid Search across {len(sample_dates)} weekly epochs in the last 3 months...")
print("Target Horizon: 2-Week Forward Returns (Short Term Trades)")
print("==============================================\n")

weights_to_test = [
    (0.3, 0.7),
    (0.4, 0.6),
    (0.5, 0.5),
    (0.6, 0.4),
    (0.7, 0.3),
    (0.8, 0.2),
    (0.9, 0.1)
]

best_ret = -999.0
best_w = None

for w_rs, w_mom in weights_to_test:
    returns = []
    
    for date in sample_dates:
        target_date = date.strftime("%Y-%m-%d")
        for ticker in stocks:
            series = stock_data[ticker]
            hist_data = series[series.index <= target_date]
            if len(hist_data) < 65: continue
            
            nifty_hist = nifty[nifty.index <= target_date]
            if len(nifty_hist) < 65: continue
            
            current_price = hist_data.iloc[-1]
            stock_63d_ret = (current_price / hist_data.iloc[-63]) - 1
            nifty_63d_ret = (nifty_hist.iloc[-1] / nifty_hist.iloc[-63]) - 1
            
            future_data = series[series.index > target_date]
            if len(future_data) < 7: continue 
            forward_return = (future_data.iloc[6] / current_price) - 1 # ~1.5 weeks forward
            
            packet = DataPacket(ticker=ticker, timestamp=datetime.utcnow())
            packet.signals.append(Signal("STOCK_63D_RET", SignalCategory.PRICE, SignalSubSource.PRICE_ACTION, stock_63d_ret, datetime.utcnow()))
            packet.signals.append(Signal("NIFTY_63D_RET", SignalCategory.PRICE, SignalSubSource.PRICE_ACTION, nifty_63d_ret, datetime.utcnow()))
            packet.signals.append(Signal("SECTOR_63D_RET", SignalCategory.PRICE, SignalSubSource.PRICE_ACTION, nifty_63d_ret, datetime.utcnow()))
            
            rs_score = RelativeStrengthEngine().evaluate(packet)['score']
            mom_score = MomentumBullEngine().evaluate(packet)['score']
            
            weighted_score = (rs_score * w_rs) + (mom_score * w_mom)
            
            if weighted_score > 38.0:
                returns.append(forward_return)
                
    if len(returns) == 0:
        continue
        
    avg_ret = np.mean(returns) * 100
    win_rate = (len([r for r in returns if r > 0]) / len(returns)) * 100
    
    print(f"Weights [RS: {int(w_rs*100)}% | Mom: {int(w_mom*100)}%] -> Win Rate: {win_rate:5.1f}% | Avg Return: {avg_ret:5.2f}% (Total Trades: {len(returns)})")
    
    if avg_ret > best_ret:
        best_ret = avg_ret
        best_w = (w_rs, w_mom)

print("==============================================")
if best_w:
    print(f"=> OPTIMAL 3-MONTH WEIGHTS: Relative Strength = {int(best_w[0]*100)}% | Momentum = {int(best_w[1]*100)}%")
else:
    print("=> No profitable trades found in the last 3 months.")
print("==============================================")
